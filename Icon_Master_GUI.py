...
# Define the application's current version
APP_VERSION = "1.0.0"


class TextboxHandler(logging.Handler):
    """A custom logging handler that redirects logs to a CTkTextbox."""
...
class IconMasterApp(ctk.CTk):
    """The redesigned GUI application that uses the IconConverter backend."""

    def __init__(self):
        super().__init__()

        self.title(f"Icon Master GUI v{APP_VERSION}")
        self.geometry("800x600")  # Larger, more modern window size
        ctk.set_appearance_mode("Dark")
        ctk.set_default_color_theme("blue")

        # --- Main Layout Configuration ---
        self.grid_columnconfigure(0, weight=1)
        self.grid_rowconfigure(2, weight=1)  # Make the log area expandable

        # --- Load Icons ---
        self.folder_icon = ctk.CTkImage(Image.open("assets/folder_icon.png"))
        self.save_icon = ctk.CTkImage(Image.open("assets/save_icon.png"))
        self.palette_icon = ctk.CTkImage(Image.open("assets/palette_icon.png"))
        self.magic_wand_icon = ctk.CTkImage(Image.open("assets/magic_wand_icon.png"))
        self.update_icon = ctk.CTkImage(Image.open("assets/update_icon.png"))
    ...
    def _create_widgets(self):
        """Create and arrange all the new widgets in a modern layout."""
    ...
    def _run_updater(self):
        # ... (This method is unchanged)
        logging.info("Checking for updates... Relaunching via updater.")
        subprocess.Popen([sys.executable, 'autoupdater.py', APP_VERSION])
        self.destroy()
